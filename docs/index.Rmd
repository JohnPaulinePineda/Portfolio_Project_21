...
|
| [NMF](https://projecteuclid.org/journals/pacific-journal-of-mathematics/volume-1/issue-2/On-the-approximation-of-a-function-of-several-variables-by/pjm/1103052191.full) decomposes multivariate data by creating a user-defined number of features where each feature is a linear combination of the original attribute set with the coefficients of these linear combinations being non-negative. The algorithm obtains lower rank factorized matrices W (feature matrix) and H (coefficient matrix) whose product approximates the original data set through an iterative procedure which modifies their initial values. The procedure terminates when the approximation error converges or the specified number of iterations is reached. The result represents the original data set mapped into the new set of attributes (features).
|

---
title: 'Unsupervised Learning : Extracting Information Using Dimensionality Reduction Algorithms'
...
# **1. Table of Contents**
|
| This project explores different dimensionality reduction algorithms for extracting information using various helpful packages in <mark style="background-color: #CCECFF">**R**</mark>. Methods applied in the analysis to transform and reduce high dimensional data included the **Principal Component Analysis**, **Singular Value Decomposition**, **Independent Component Analysis**, **Non-Negative Matrix Factorization**, **t-Distributed Stochastic Neighbor Embedding** and **Uniform Manifold Approximation and Projection** algorithms. The various algorithms were subjectively evaluated based on their capability to effectively capture the latent characteristics between the different resulting components. All results were consolidated in a [<span style="color: #FF0000">**Summary**</span>](#summary) presented at the end of the document.
|
...
|
| [Singular Value Decomposition](https://link.springer.com/article/10.1007/BF02288367) decomposes the original data set (m x n matrix) into three component matrices factorized in the form USV where U, S and V correspond to an m x p matrix, a p x p diagonal matrix and a p x n matrix, respectively. The value p is called the rank. The diagonal entries of S are referred to as the singular values of M. The columns of U are typically called the left-singular vectors of M, and the columns of V are called the right-singular vectors of M. Given the decomposition into the U, S and V matrices, the algorithm is able to reconstruct the original data set or an approximation of it. The singular values in the diagonal matrix S provide the amount of variance explained by each of the singular vectors. This information is used to limit the number of vectors to the reduced amount of variance aimed to be captured. 
|
...
|
| [Independent Component Analysis](https://www.sciencedirect.com/science/article/abs/pii/0165168494900299) identifies a set of basis functions that can be used to represent the original data set by searching for a linear transformation that minimizes the statistical dependence between its components. The identified basis functions which are chosen to be statistically independent and non-Gaussian are used to separate the original data into a set of independent components.
|
...
|
| [Non-Negative Matrix Factorization](https://projecteuclid.org/journals/pacific-journal-of-mathematics/volume-1/issue-2/On-the-approximation-of-a-function-of-several-variables-by/pjm/1103052191.full) decomposes the original data set (m x n matrix) by creating a user-defined number of features where each feature is a linear combination of the original attribute set with the coefficients of these linear combinations being non-negative. The algorithm obtains lower rank factorized matrices W (m x k feature matrix) and H (k x n coefficient matrix) whose product approximates the original data set through an iterative procedure which modifies their initial values. The values of k represent the low rank approximation of the original data set. The procedure terminates when the approximation error converges or the specified number of iterations is reached. The result represents the original data set mapped into the new set of attributes (features).
|
...
|
| [t-Distributed Stochastic Neighbor Embedding](https://www.semanticscholar.org/paper/Visualizing-Data-using-t-SNE-Maaten-Hinton/1c46943103bd7b7a2c7be86859995a4144d1938b) identifies patterns in the data based on the similarity of data points with features by creating a probability distribution that captures these mutual distance relationships between the points in the initial high-dimensional space. The algorithm then minimizes the difference between these conditional probabilities (or similarities) in higher-dimensional and lower-dimensional space for a perfect representation of data points in lower-dimensional space. Stochastic gradient descent optimization is applied with a cost function using the Kullback–Leibler divergence — a commonly used measure of how different two data distributions are.
|
...
|
| [Uniform Manifold Approximation and Projection](https://www.semanticscholar.org/paper/UMAP%3A-Uniform-Manifold-Approximation-and-Projection-McInnes-Healy/3a288c63576fc385910cb5bc44eaea75b442e62e) constructs a fuzzy topological representation and optimizes the low dimensional representation as measured by cross entropy. When constructing the initial fuzzy topological representation, fuzzy set membership strengths are only computed for the nearest neighbors of each point using the Nearest-Neighbor-Descent algorithm for efficiency. Optimizing the low dimensional embedding applies the stochastic gradient descent process using a differentiable objective function involving a smooth approximation of the actual membership strength function for the low dimensional representation, selecting from a suitably versatile family. Since the Laplacian of the topological representation is an approximation of the Laplace-Beltrami operator of the manifold, spectral embedding techniques are also used to initialize the low dimensional representation into a good state.
|
...
|
| ![](D:/Github_Codes/ProjectPortfolio/Portfolio_Project_21/images/Project21_Summary.png)
|
# **3. References**
|
...
| **[Article]** [Singular Value Decomposition (SVD)](https://www.machinelearningmindset.com/singular-value-decomposition-svd/) by Machine Learning Mindset Team
...
| **[Article]** [Explained: Singular Value Decomposition (SVD)](https://storrs.io/svd/) by Erik Storrs
| **[Article]** [Using SVD for Dimensionality Reduction](https://blogs.oracle.com/machinelearning/post/using-svd-for-dimensionality-reduction) by Mark Hornick
...
| **[Publication]** [The Approximation of One Matrix by Another of Lower Rank](https://link.springer.com/article/10.1007/BF02288367) by Carl Eckart and Gale Young (Psychometrika) 
| **[Publication]** [Independent Component Analysis, A New Concept?](https://www.sciencedirect.com/science/article/abs/pii/0165168494900299) by Pierre Comon (Signal Processing)
| **[Publication]** [On the Approximation of a Function of Several Variables by the Sum of Functions of Fewer Variables](https://projecteuclid.org/journals/pacific-journal-of-mathematics/volume-1/issue-2/On-the-approximation-of-a-function-of-several-variables-by/pjm/1103052191.full) by Author (Pacific Journal of Mathematics)
| **[Publication]** [Visualizing Data using t-SNE](https://www.semanticscholar.org/paper/Visualizing-Data-using-t-SNE-Maaten-Hinton/1c46943103bd7b7a2c7be86859995a4144d1938b) by Laurens van der Maaten and Geoffrey Hinton (Journal of Machine Learning Research)
| **[Publication]** [UMAP: Uniform Manifold Approximation and Projection for Dimension Reduction](https://www.semanticscholar.org/paper/UMAP%3A-Uniform-Manifold-Approximation-and-Projection-McInnes-Healy/3a288c63576fc385910cb5bc44eaea75b442e62e) by Leland McInnes and John Healy (Computer Science)
